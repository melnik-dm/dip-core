...
public class SpellCheckIterator implements ISpellCheckIterator {
	...
	protected String nextToken() {
		...
		if (fNext - fPrevious > 0) {
			...
			if (fSuccessor != BreakIterator.DONE && fContent.charAt(fPrevious) == HtmlTagConstants.HTML_TAG_PREFIX && (Character.isLetter(fContent.charAt(fNext)) || fContent.charAt(fNext) == '/')) {

				if (fContent.startsWith(HtmlTagConstants.HTML_CLOSE_PREFIX, fPrevious))
					nextBreak();

				nextBreak();

				if (fSuccessor != BreakIterator.DONE && fContent.charAt(fNext) == HtmlTagConstants.HTML_TAG_POSTFIX) {
			...
			} else if (fSuccessor != BreakIterator.DONE && fContent.charAt(fPrevious) == HtmlTagConstants.HTML_ENTITY_START && (Character.isLetter(fContent.charAt(fNext)))) {
				nextBreak();
				if (fSuccessor != BreakIterator.DONE && fContent.charAt(fNext) == HtmlTagConstants.HTML_ENTITY_END) {
					nextBreak();
					if (isToken(fContent.substring(fPrevious, fNext), HtmlTagConstants.HTML_ENTITY_CODES)) {
						skipTokens(fPrevious, HtmlTagConstants.HTML_ENTITY_END);
						update= true;
					} else
						token= fContent.substring(fPrevious, fNext);
				} else
