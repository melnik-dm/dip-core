...
/**
 * Класс Parser из библиотеки commonmark (Класс AsciiMatcher из библиотеки commonmark (https://opensource.org/license/BSD-2-Clause))
 * Переопределен метод createDocumentParser(), чтобы изменить парсер в части необходимости пустых 
 * строк перед списком
 */
...
public class MdParser {
	...
	public static MdParser instance() {
		if (instance == null) {
			instance = MdParser.builder().inlineParserFactory(new InlineParserFactory() {
				
				@Override
				public InlineParser create(InlineParserContext inlineParserContext) {
					return new MdInlineParserImpl(inlineParserContext);
				}
			}).build();
		}
		return instance;
	...
	private MdParser(Builder builder) {
		...
		this.inlineParserFactory
				.create(new InlineParserContextImpl(delimiterProcessors, new LinkReferenceDefinitions()));
	}
	 ...
	 */
	public Node parse(String input, IDipUnit unit) {
		if (input == null) {
			throw new NullPointerException("input must not be null");
		}
		MdDocumentParser documentParser = createDocumentParser(unit);
	 ...
	 * it before handing the reader to this library. There's existing classes that
	 * do that, e.g. see {@code BOMInputStream} in Commons IO.
	 * <p>
	 * This method is thread-safe (a new parser state is used for each invocation).
	 *
	 ...
	 */
	public Node parseReader(Reader input, IDipUnit unit) throws IOException {
		if (input == null) {
			throw new NullPointerException("input must not be null");
		}
		MdDocumentParser documentParser = createDocumentParser(unit);
	...
	private MdDocumentParser createDocumentParser(IDipUnit unit) {
		return new MdDocumentParser(blockParserFactories, inlineParserFactory, delimiterProcessors, includeSourceSpans, unit);
	...
	public static class Builder {
		 ...
		 * </pre>
		 *
		 * @param enabledBlockTypes A list of block nodes the parser will parse. If this
		 *                          list is empty, the parser will not recognize any
		 ...
		 * factories. This is so that extensions can change how some syntax is parsed
		 * that would otherwise be handled by built-in factories. "With great power
		 ...
		 * added, as long as they have a different minimum length. In that case, the
		 * processor with the shortest matching length is used. Adding more than one
		 * delimiter processor with the same character and minimum length is invalid.
		 *
		 ...
		 * inline parser to modify how the following are parsed: bold (**) italic (*)
		 * strikethrough (~~) backtick quote (`) link ([title](http://)) image
		 * (![alt](http://))
		 * <p>
		 * Note that if this method is not called or the inline parser factory is set to
		 * null, then the default implementation will be used.
		 *
