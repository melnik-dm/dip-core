 ...
 * set WordWrap mode
 * 
 * This work is derived from the original source code of the MergeSourceViewer.java.
 *
 * 
 *******************************************************************************/
...
public class WrapMergeSourceViewer
	...
	class TextEditorAdapter implements ITextEditor {
		...
		public IDocumentProvider getDocumentProvider() {
			return new IDocumentProvider() {
				...
				public IDocument getDocument(Object element) {
					return WrapMergeSourceViewer.this.getSourceViewer().getDocument();
				}
		...
		public ISelectionProvider getSelectionProvider() {
			return WrapMergeSourceViewer.this.getSourceViewer().getSelectionProvider();
		}
		...
		private void selectAndReveal(int selectionStart, int selectionLength, int revealStart, int revealLength) {
			...
			StyledText widget = WrapMergeSourceViewer.this.getSourceViewer().getTextWidget();
			widget.setRedraw(false);
			adjustHighlightRange(revealStart, revealLength);
			WrapMergeSourceViewer.this.getSourceViewer().revealRange(revealStart, revealLength);

			WrapMergeSourceViewer.this.getSourceViewer().setSelectedRange(selectionStart, selectionLength);
		...
		private void adjustHighlightRange(int offset, int length) {
			...
			if (WrapMergeSourceViewer.this instanceof ITextViewerExtension5) {
				ITextViewerExtension5 extension = (ITextViewerExtension5) WrapMergeSourceViewer.this;
				extension.exposeModelRange(new Region(offset, length));
			} else if (!isVisible(WrapMergeSourceViewer.this.getSourceViewer(), offset, length)) {
				WrapMergeSourceViewer.this.getSourceViewer().resetVisibleRegion();
			}
		}
		...
		public IEditorInput getEditorInput() {
			if (WrapMergeSourceViewer.this.fContainer.getWorkbenchPart() instanceof IEditorPart)
				return ((IEditorPart) WrapMergeSourceViewer.this.fContainer.getWorkbenchPart()).getEditorInput();
			return null;
		}
		...
		public IWorkbenchPartSite getSite() {
			return WrapMergeSourceViewer.this.fContainer.getWorkbenchPart().getSite();
		}
...
// line number ruler support
	...
	public WrapMergeSourceViewer(SourceViewer sourceViewer, ResourceBundle bundle, ICompareContainer container) {
...
// for listening to editor show/hide line number preference value
		fPreferenceChangeListener = event -> WrapMergeSourceViewer.this.handlePropertyChangeEvent(event);
...
//}
	...
	public Point getLocationRange(Position p, Point region) {
		StyledText st = getSourceViewer().getTextWidget();
		if (p == null || /* doc */st == null) {
			region.x = 0;
			region.y = 0;
			return region;
		}
		...
		int startY = st.getLocationAtOffset(start).y;

		int end = start + length - 1;
		if (end < 0) {
			end = 0;
		}
		int endY = st.getLocationAtOffset(end).y;

		return new Point(startY, endY - startY);
	}

	/*
	 * Returns the start line and the number of lines which correspond to the given
	 * position. Starting line number is 0 based.
	 */
...
//			}
	...
	protected IAction createAction(String actionId) {
		if (UNDO_ID.equals(actionId))
			return new TextOperationAction(ITextOperationTarget.UNDO, IWorkbenchCommandConstants.EDIT_UNDO, true, false,
					true);
		if (REDO_ID.equals(actionId))
			return new TextOperationAction(ITextOperationTarget.REDO, IWorkbenchCommandConstants.EDIT_REDO, true, false,
					true);
		...
		if (COPY_ID.equals(actionId))
			return new TextOperationAction(ITextOperationTarget.COPY, IWorkbenchCommandConstants.EDIT_COPY, false, true,
					false);
...
// to get undo redo right
		...
		if (handler != null) {
			if (fSaveContributionItem == null) {
				fSaveContributionItem = new CommandContributionItem(
						new CommandContributionItemParameter(fContainer.getWorkbenchPart().getSite(), null,
	 ...
	 * have to be called it manually. The wrapped viewer is disposed as a regular
	 * viewer, while disposing the UI.
	 */
	public void dispose() {
