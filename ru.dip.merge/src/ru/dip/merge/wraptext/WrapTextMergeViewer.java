 ...
 */
public class WrapTextMergeViewer extends WrapContentMergeViewer implements IAdaptable {
	...
	class ContributorInfo implements IElementStateListener, VerifyListener, IDocumentListener, IEncodingSupport {
				...
				if (parent instanceof IDocumentRange) {
					...
					if (range == null) {
						...
						try {
							...
							// silently ignored
							if (WrapTextMergeViewer.DEBUG) System.out.println("BadPositionCategoryException: " + ex);	//$NON-NLS-1$
						} catch (BadLocationException ex) {
							// silently ignored
							if (WrapTextMergeViewer.DEBUG) System.out.println("BadLocationException: " + ex);	//$NON-NLS-1$
						}
		...
		private void updateViewerDocumentRange(WrapMergeSourceViewer tp, Position range) {
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					IRegion r= this.fViewer.normalizeDocumentRegion(tp.getSourceViewer().getDocument(), WrapTextMergeViewer.toRegion(range));
		...
		private void updateViewerDocument(WrapMergeSourceViewer tp, IDocument document, Position range) {
			...
			try {
				if (this.fViewer.fSubDoc && range != null) {
					IRegion r= this.fViewer.normalizeDocumentRegion(document, WrapTextMergeViewer.toRegion(range));
		...
		public void documentChanged(DocumentEvent e) {
			...
			WrapTextMergeViewer.this.documentChanged(e, dirty);
	 ...
	 */
	public WrapTextMergeViewer(Composite parent, CompareConfiguration configuration) {
	...
	public WrapTextMergeViewer(Composite parent, int style, CompareConfiguration configuration) {
		...
		fMerger = new DocumentMerger(new IDocumentMergerInput() {
			...
			public int getHunkStart() {
				return WrapTextMergeViewer.this.getHunkStart();
			}
			@Override
			...
			public boolean isIgnoreAncestor() {
				return WrapTextMergeViewer.this.isIgnoreAncestor();
			}
			@Override
			public boolean isPatchHunk() {
				return WrapTextMergeViewer.this.isPatchHunk();
			}
			...
			public boolean isThreeWay() {
				return WrapTextMergeViewer.this.isThreeWay();
			}
			@Override
			public boolean isPatchHunkOk() {
				return WrapTextMergeViewer.this.isPatchHunkOk();
			}
		});
