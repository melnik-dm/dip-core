 ...
 * 		2025 Ruslan Sabirov.
 * 		2025 Andrei Motorin.
 * Changed access to class fields
 * 
 * This work is derived from the original source code of the ContentMergeViewer.java.
 *
 *     
 *******************************************************************************/
...
/**
 * Копия ContentMergeViewer
 * Нужны были поля с доступом package
 */
public abstract class WrapContentMergeViewer extends ContentViewer
	 ...
	 */
	protected WrapContentMergeViewer(int style, ResourceBundle bundle, CompareConfiguration cc) {
		if (Policy.debugContentMergeViewer) {
	...
	protected final Control buildControl(Composite parent) {
		fComposite= new Composite(parent, fStyles | SWT.LEFT_TO_RIGHT) { // We force a specific direction
			...
			public boolean setFocus() {
				return WrapContentMergeViewer.this.handleSetFocus();
			}
		};
