...
public class WrapShowWhitespaceAction extends WrapTextEditorPropertyAction {

	private Map<WrapMergeSourceViewer, WhitespaceCharacterPainter> fPainters;
	...
	public WrapShowWhitespaceAction(WrapMergeSourceViewer[] viewers, boolean[] needsPainters) {
	...
	private synchronized Map<WrapMergeSourceViewer, WhitespaceCharacterPainter> getPainters() {
		if (fPainters == null)
	...
	private void showWhitespace() {
		if (isWhitespaceShowing)
			return;
		try {
			Map<WrapMergeSourceViewer, WhitespaceCharacterPainter> painters = getPainters();
			WrapMergeSourceViewer[] viewers = getViewers();
			for (int i = 0; i < viewers.length; i++) {
				if (fNeedsPainters[i]) {
					WrapMergeSourceViewer viewer = viewers[i];
	...
	private void hideWhitespace() {
		Map<WrapMergeSourceViewer, WhitespaceCharacterPainter> painters = getPainters();
		for (WrapMergeSourceViewer viewer : painters.keySet()) {
