...
public class NewDemoProjectWizard extends Wizard implements INewWizard {

	private NewDemoProjectPage fPage;
	private boolean fPerformFinish = false;
	
	public NewDemoProjectWizard() {
	    setWindowTitle(Messages.NewDemoProjectWizard_Title);
	}
	
	@Override
	public void addPages() {
		fPage = new NewDemoProjectPage(Messages.NewDemoProjectWizard_DemoProjectPage, Messages.NewDemoProjectWizard_DempProjectDescription, Messages.NewDemoProjectWizard_demo);
		addPage(fPage);
	}
		...
		return new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				Display.getDefault().asyncExec(() -> {
					...
					DipProject dipProject = DipUtilities.createDemoProject(name, getShell());		
					if (dipProject != null) {
						dipProject.refresh();
